    ...
    implements GraphObserver
{
    ...
    public void addGraficalEdge( GraficalEdge graficalEdge )
                         throws Exception
    {
        ...
        } else
        {
            ...
            edgeLine.paint( this.getGraphics(  ),
                            this.defaultColor );
        }
    }

    public void highlightGraficalEdge( int startVertex, int endVertex, int cost, Color color )
    {
        try
        {
            GraficalEdge otherGraficalEdge;
            GraficalEdge graficalEdge = GraficalEdge.getZeroGraficalEdge( startVertex, endVertex, cost );
            ...
            if ( contains )
            {
                ...
                edgeLine.paint( super.getGraphics(  ),
                                color );
            ...
            switch ( panelState )
            {
                ...
                case PUT_VERTEX:
                {
                    ...
                    if ( vertexToPut < vertexQtyToPut )
                    {
                        ...
                        GraficalVertex grafVertex =
                            new GraficalVertex( integer.toString(  ), vertexColor, vertexFont,
                    ...
                    } else if ( vertexToPut >= vertexQtyToPut )
                    {
                        if ( vertexToPut == ( vertexQtyToPut ) )
                        {
                            ...
                            vertexToPut++;
                            JOptionPane.showMessageDialog( jPanel, "babsonicos putos" );
                        } else
                        {
                            JOptionPane.showMessageDialog( jPanel, GraphView2.VERTEX_QTY_ERROR );
                        }
                    }
                ...
                case TEST_GRAFICAL_EDGE:
                {
                    try
                    {
                        ...
                        int cost = 10; // 0---->1
                        GraficalEdge graficalEdge = new GraficalEdge( startPoint, endPoint, 0, 1, 10 );
    ...
    public void vertexDeleted( int v )
    {
        ...
        for ( int indexToOverwrite = ( v + 1 ); indexToOverwrite < newVertexArray.length; indexToOverwrite++ )
        {
            ...
            newVertexArray[indexToOverwrite - 1].setVertex( indexToOverwrite - 1 );
        }
    ...
    public void weightAdded( int startVertex, int endVertex, int weight )
    {
        ...
        GraficalEdge graficalEdge = new GraficalEdge( startPoint, endPoint, startVertex, endVertex, weight );
    ...
    {
        GraficalEdge graficalEdge = GraficalEdge.getZeroGraficalEdge( startVertex, endVertex, weight );
    ...
    { // tenemos
        ...
        for ( int j = 0; j < graficalListEdge.size(  ); j++ )
        {
            ...
            {
                if ( graficalListEdge.get( j )
                                         .equals( GraficalEdge.getZeroGraficalEdge( ( (Integer) vertex.get( i ) ),
                                                                                        (Integer) vertex.get( i + 1 ),
                                                                                        (Integer) cost.get( i ) ) ) )
                {
                    highlightGraficalEdge( graficalListEdge.get( j ),
                                           specialColor );
                }
            }
        }
