...
public class AdjacentListWeightGraph<T> implements Undirected<T>, IWeight {
	private int vertexQty;
	private int edgeQty;
	private List<T> vertexObjectList;
	// compare by vertex criteria
	private Comparator<Pair<Integer, Integer>> comparatorPair;

	// compare with weight of the vertex criteria
	...
	public AdjacentListWeightGraph(int vertexQty) throws Exception {
	...
	private class ComparatorPairWeight implements
	...
	private class ComparatorPair implements Comparator<Pair<Integer, Integer>> {

		public int compare(Pair<Integer, Integer> arg0,
				Pair<Integer, Integer> arg1) {
			// TODO Auto-generated method stub
			// the first index is vertex and is valid

			return (arg0.getFirst() - arg1.getFirst());
		}
	}

	private class ComparatorInteger implements Comparator<Integer> {
	...
	private List<Integer> subGetAdjacentList(int vertex) {
		...
		Iterator<Pair<Integer, Integer>> iterator = adjacentList[vertex]
				.iterator();
		...
		if (position >= 0) {
			...
			adjacentListAnswer.removeAll(Collections.singleton(new Integer(
					vertex)));
		}
	...
	private void subDeleteVertexWithLoop(int vertexToDelete) throws Exception {
		int oldVertexQty = vertexQty;
		Boolean check;
		for (int i = vertexToDelete + 1; i >= (vertexToDelete + 1)
				&& (i < oldVertexQty); i++) {
	...
	private void subDeleteVertex(int vertexToDelete) throws Exception {
	...
	private void subDeleteVertexObjectList(int vertexToDelete) throws Exception {
		int oldVertexQty = vertexQty;
	...
	private void decreaseVertexInAdyacentList(int vertex) throws Exception {
		List listPair;
	 ...
	 * @throws Exception
	 */
	private void resetVertexWithLoop() throws Exception {
		int position;
	...
	public void deleteVertex(int vertexToDelete) throws Exception {
		...
		if (isInvalidVertex(vertexToDelete) || (vertexHasAdjacent)) {
			...
			/*
			 * Iterator<GraphObserver> iterator=graphObserverList.iterator();
			 * GraphObserver graphObserver; while(iterator.hasNext()){
			 * 
			 * graphObserver=iterator.next();
			 * graphObserver.vertexDeleted(vertexToDelete);
			 * 
			 * 
			 * }
			 */
	...
	private void subAddEdge(int startVertex, int endVertex) throws Exception {
	...
	private boolean subExistEdge(int startVertex, int endVertex) {
		...
		return (subExistDirectEdge(startVertex, endVertex) || subExistDirectEdge(
	...
	private int vertexDelete(List<Pair<Integer, Integer>> list,
			Integer vertexToDelete) throws Exception {
		int qty = 0;
	...
	private void subDeleteEdge(int startVertex, int endVertex) throws Exception {
		if (startVertex == endVertex) {
	...
	private List<Pair<Integer, Integer>> subGetAdjcentListWithWeight(int vertex) {
		...
		List<Pair<Integer, Integer>> adjacentListAnswer = new ArrayList<Pair<Integer, Integer>>(
				vertexQty);
	...
	private void subSetWeight(int startVertex, int endVertex, int weight,
			int newWeight) {
