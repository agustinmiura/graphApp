...
public class AdjacentListGraph<T> implements Undirected<T> {
	...
	public AdjacentListGraph(int vertexQty) throws Exception {
	...
	private void subDeleteVertexWithLoop(int vertexToDelete) throws Exception {
		...
		/**
		 * piso el vertice con loop {2,2} por ejemplo para borrar ese vertice
		 */
		Boolean check;
	...
	private void removeFromAdjacentList(int vertex) throws Exception {

		for (int i = 0; i < vertexQty; i++) {

			adyacentList[i].removeAll(Collections
					.singleton(new Integer(vertex)));
		}
	}
	...
	private void subDeleteVertex(int vertexToDelete) throws Exception {
		int oldVertexQty = vertexQty;
		// overwrite the integer list in position vertex to
		// delete
	...
	public void deleteVertex(int vertexToDelete) throws Exception {
		...
		if (isInvalidVertex(vertexToDelete)
				|| (!adyacentList[vertexToDelete].isEmpty())) {

			throw new Exception(
					"bad vertex to delete or the vertex has adjacent vertex");
	...
	private void subDeleteEdge(int startVertex, int endVertex) throws Exception {
		...
		if (subExistDirectEdge(startVertex, endVertex)) {
			...
			positionFound = Collections.binarySearch(adyacentList[startVertex],
					endVertex);
		...
		if (subExistDirectEdge(endVertex, startVertex)) {
			...
			positionFound = Collections.binarySearch(adyacentList[endVertex],
					startVertex);
	 ...
	 * @return
	 */
	private T subGetVertexObject(int vertex) throws ClassCastException {
		Object object = null;
		T t = null;
