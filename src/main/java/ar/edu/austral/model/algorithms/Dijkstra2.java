...
public class Dijkstra2<T>
{
    ...
    public Dijkstra2( IGraph grafo, int v )
              throws Exception
    {
        int nodo_origen = v + 1;
        ...
        for ( int i = 0; i < resultado.length; i++ )
        {
            ...
            System.out.println( "v" + ( 
                                          resultado[i].camino.get( resultado[i].camino.size(  ) - 1 ).intValue(  ) + 1
                                       ) + "\n" );
        }
    }
    ...
    private NodoA[] resolver( IGraph grafo, int nodo_origen )
                      throws Exception
    {
        ...
        while ( ! soluciones.isEmpty(  ) )
        {
            ...
            for ( int i = 0; i < n; i++ )
            {
                ...
                if ( ( ( in + origen.distancia ) < retorno[i].distancia ) || ( retorno[i].distancia == 0 ) )
                {
