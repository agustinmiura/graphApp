...
import ar.edu.austral.model.interfaces.*;
...
public class GraphShowApp extends JFrame {
	...
	private JMenu createModifyGraphMenu() {
		...
		menuItem = new JMenuItem("Add vertex ");// TODO
	...
	private class DeleteVertexACtion implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				String stringInteger = JOptionPane.showInputDialog(jFrame,
						GraphAppConstants.DIALOG_MESSAGE_ENTER_VERTEX);
	...
	private class CommonDeleteVertexPaint implements ActionListener {
		@Override
	...
	private class CommonDeleteEdgePaint implements ActionListener {

		@Override
		public void actionPerformed(ActionEvent e) {

			DeleteEdgeDialog d = new DeleteEdgeDialog(jFrame, graphView2,
					(IWeight) iGraph);
	...
	private class CommonVertexPaint implements ActionListener {
	...
	private class CommonEdgePaint implements ActionListener {

		private boolean usingInvalidVertex(int startVertex, int endVertex,
				int cost) {

			boolean check0 = (startVertex < 0 || startVertex > iGraph
					.getVertexQty());
			boolean check1 = (endVertex < 0 || endVertex > iGraph
					.getVertexQty());
			boolean check2 = (iGraph == null);
		...
		public void actionPerformed(ActionEvent e) {
			...
			AddEdgeDialog a = new AddEdgeDialog(jFrame, graphView2,
					(IWeight) iGraph);
	...
	private class SpecialEdgePaint implements ActionListener {

		@Override
		public void actionPerformed(ActionEvent e) {
			try {

				String stringStartVertex = JOptionPane.showInputDialog(this,
						"get startVertex");
				String stringEndVertex = JOptionPane.showInputDialog(this,
						"get endVertex");
				String stringCost = JOptionPane.showInputDialog(this,
						"get cost");
				...
				graphView2.highlightGraficalEdge(startVertex, endVertex, cost,
	...
	private class RestoreConfig implements ActionListener {
	...
	private class DoFloyd implements ActionListener {
			...
			if (check) {
				...
				try {
					...
					TableDialog tableDialog = new TableDialog(jFrame,
							solveList.get(0), solveList.get(1));
				} catch (Exception e1) {
					e1.printStackTrace();
				}
	...
	private class CreateGraph implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			stringInteger = JOptionPane
					.showInputDialog(GraphAppConstants.DIALOG_MESSAGE_VERTEX_QTY);
			vertexQty = new Integer(stringInteger);

			int n = JOptionPane.showOptionDialog(jFrame,
					GraphAppConstants.DIALOG_MESSAGE_CREATE_G,
					GraphAppConstants.DIALOG_NAME_CREATE_G,
					JOptionPane.YES_NO_CANCEL_OPTION,
					JOptionPane.QUESTION_MESSAGE, null,
					GraphAppConstants.GRAPH_NAME, // the titles of buttons
			...
			case 0: {
				try {
					...
					JOptionPane
							.showMessageDialog(
									jFrame,
									GraphAppConstants.DIALOG_MESSAGE_INVALID_VERTEX_QTY);
				}
			}
			case 1: {
				try {
	...
	private class DeleteGraph implements ActionListener {
	...
	private class ResetGraph implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (invalidParameter) {
				JOptionPane.showMessageDialog(jFrame,
						GraphAppConstants.DIALOG_MESSAGE_ERROR);
				return;
			} else {
	...
	private class DoPrim implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				String stringVertex = JOptionPane.showInputDialog(jFrame,
						GraphAppConstants.DIALOG_MESSAGE_ENTER_VERTEX);
				Integer startVertex = new Integer(stringVertex);
				if (startVertex < 0 || startVertex > iGraph.getVertexQty()) {
					JOptionPane.showMessageDialog(jFrame,
							GraphAppConstants.MESSAGE_ERROR_INVALID_VERTEX);
					return;
				}
				...
				if (invalidParameter) {
					...
					JOptionPane.showMessageDialog(jFrame,
							GraphAppConstants.DIALOG_MESSAGE_ERROR_PRIM);
				} else {
					int[] solutionArray = MinimumSpanningTreePrim2
							.subGetFatherArray((IWeight) iGraph, startVertex);

					MyPrimDialog myPrimDialog = new MyPrimDialog(jFrame,
							MyPrimDialog.getDialogPrimActions(), solutionArray,
							iGraph, graphView2);
				}
	...
	private class DoDijstra implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				String message = JOptionPane.showInputDialog(this,
						"Please write the beginning vertex");
				beginningVertex = Integer.parseInt(message);
				if (beginningVertex < 0
						|| beginningVertex > (iGraph.getVertexQty() - 1)) {
					return;
				}
			} catch (NumberFormatException f) {
				new JOptionPane(
						"Valor invalido, el vertice inicial debe ser el indice del vertice.");
			}
			...
			for (int i = 0; i < resultado.length; i++) {
				list.add(new String("Desde v" + (nodo_origen) + " hasta v"
						+ (i) + ": " + resultado[i].distancia));
			}
			MyListDialog listDialog = new MyListDijkstraDialog(jFrame, list,
					resultado, nodo_origen, g, d);
